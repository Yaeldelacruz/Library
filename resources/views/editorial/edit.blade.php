...
@section('accion')
    {{ route('editorial.update', $editorial) }}
@endsection
...
@section('body')
    <div class="grid grid-cols-4 grid-flow-row gap-4">
        <div>
            ...
            <input type="text" name="name" value="{{ old('name',$editorial->name) }}" class="text-gray-600 text-base font-semibold my-2 px-4 py-2 rounded-md border border-gray-600 hover:border-green-600 focus:ring-1 w-full">
            @error('name')
        ...
        <div class="text-center">
            <button type="submit" class="text-green-600 text-lg border border-green-600 bg-white hover:bg-green-600 hover:text-white rounded-md px-4 py-2 my-10">Actualizar editorial</button>
        </div>
    </div>
@endsection