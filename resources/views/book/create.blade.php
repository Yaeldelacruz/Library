...
<div class="grid grid-cols-3 grid-flow-row gap-4">
    ...
    <div>
        ...
        <input type="number" name="count" value="{{ old('count') }}" class="text-gray-600 text-base font-semibold my-2 px-4 py-2 rounded-md border border-gray-600 hover:border-blue-600 focus:ring-1 w-full">
        @error('count')
            <p class="text-red-600 font-bold text-sm block my-2"><small>*{{ $message }}</small></p>
        @enderror
    </div>
...
<div class="w-full text-center">
    <button type="submit" class="text-blue-600 text-lg border border-blue-600 bg-white hover:bg-blue-600 hover:text-white rounded-md px-4 py-2 my-10">Subir libro</button>
</div>
@endsection